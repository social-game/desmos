...
var msgAnswerPollPost = msgs.NewMsgAnswerPoll(models.PostID(1), []models.AnswerID{1, 2}, testOwner)
...
func TestMsgAnswerPollPost_ValidateBasic(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			msg:   msgs.NewMsgAnswerPoll(models.PostID(0), []models.AnswerID{1, 2}, msgAnswerPollPost.Answerer),
			error: sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "Invalid post id"),
		},
		{
			name:  "Invalid answerer address",
			msg:   msgs.NewMsgAnswerPoll(models.PostID(1), []models.AnswerID{1, 2}, nil),
			error: sdkerrors.Wrap(sdkerrors.ErrInvalidAddress, "Invalid answerer address: "),
		},
		{
			name:  "Returns error when no answer is provided",
			msg:   msgs.NewMsgAnswerPoll(models.PostID(1), []models.AnswerID{}, msgAnswerPollPost.Answerer),
			error: sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "Provided answers must contains at least one answer"),
		},
		{
			name: "Valid message returns no error",
			msg:  msgs.NewMsgAnswerPoll(models.PostID(1), []models.AnswerID{1, 2}, msgAnswerPollPost.Answerer),
		},
	}
