...
var msgEditPost = msgs.NewMsgEditPost(models.PostID(94), "Edited post message", testOwner, editDate)
...
func TestMsgEditPost_ValidateBasic(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			msg:   msgs.NewMsgEditPost(models.PostID(0), "Edited post message", testOwner, editDate),
			error: sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "Invalid post id"),
		},
		{
			name:  "Invalid editor returns error",
			msg:   msgs.NewMsgEditPost(models.PostID(10), "Edited post message", nil, editDate),
			error: sdkerrors.Wrap(sdkerrors.ErrInvalidAddress, "Invalid editor address: "),
		},
		{
			name:  "Blank message returns error",
			msg:   msgs.NewMsgEditPost(models.PostID(10), " ", testOwner, editDate),
		...
		},
		{
			...
			msg:   msgs.NewMsgEditPost(models.PostID(10), "My new message", testOwner, time.Time{}),
			error: sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "Invalid edit date"),
		},
		{
			name:  "Future edit date returns error",
			msg:   msgs.NewMsgEditPost(models.PostID(10), "My new message", testOwner, time.Now().Add(time.Hour)),
			error: sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "Edit date cannot be in the future"),
		},
		{
			name:  "Valid message returns no error",
			msg:   msgs.NewMsgEditPost(models.PostID(10), "Edited post message", testOwner, editDate),
			error: nil,
		},
	}
